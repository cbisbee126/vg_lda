#!/usr/bin/env python3
"""
K-selection sweep for All Games (K=5-100, step 5)
Sequential training with full multicore parallelization per model.
Note: Nested parallelization doesn't work with LdaMulticore, so we train
one K at a time but use all available cores for each model.
"""
...
def main():
    ...
    K_GRID = list(range(5, 101, 5))  # [5, 10, 15, ..., 100]
    RANDOM_STATE = 11
    PASSES, ITERS = 5, 400
    CHUNKSIZE = 2000
    ...
    print("=" * 80)
    print("ALL GAMES: K-SELECTION SWEEP (K=5-100)")
    print("=" * 80)
    ...
    def plot_combined(df, best_k, out_path):
        ...
        ax1.set_title(f"LDA K Sweep (All Games, K=5-100) — Best K={best_k}")
