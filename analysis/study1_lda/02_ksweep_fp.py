#!/usr/bin/env python3
"""
K-selection sweep for FP (First-Person/Competitive) games
...
def main():
    # Config
    INPUT_FILE = os.path.join("data", "derived", "Filtered_Combined_FP_Cleaned.parquet")
    ...
    print("=" * 80)
    print("FP ANALYSIS: K-SELECTION SWEEP (K=5-50)")
    print("=" * 80)
    ...
    RESULTS_CSV = os.path.join(TABLES_DIR, "lda_k_selection_FP_metrics_5_50.csv")
    PLOT_COMBINED = os.path.join(FIGURES_DIR, "lda_k_sweep_FP_5_50.png")
    ...
    topics_csv = os.path.join(TABLES_DIR, f"best_topics_FP_k{best_k}.csv")
    ...
    def plot_combined(df, best_k, out_path):
        ...
        ax1.set_title(f"LDA K Sweep (FP Games, K=5-50) — Best K={best_k}")
